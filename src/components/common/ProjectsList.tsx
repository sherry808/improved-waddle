...
export default function ProjectsList({
  ...
  const projects = Object.values(pdpData).filter(
            ...
            <div className="p-6">
              <div className="flex gap-2 mb-2">
                {project.page1.area.map((area, i) => (
                  <span
                    key={i}
                    className="text-[12px] font-nats text-[#351A12]"
                  >
                    {area}
                  </span>
                ))}
              </div>
              <h3 className="font-normal text-[15px] font-nats text-[#351A12] mb-2">
                {project.page1.title}
              </h3>
              <p className="text-[12px] font-nats text-[#351A12]">
                {project.page1.shortDescription}
              </p>
            </div>
          </Link>
        </motion.div>
